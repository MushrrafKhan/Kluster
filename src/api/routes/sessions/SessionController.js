const {
  models: { User, Booking, Review, LikeDislike, TrainerSession, Notification, Card, Payment, Customer, Group, JoinGroup, Bank, Chat, Filter }
} = require('../../../../app/models');
...
class SessionController {
  ...
  async TrainersList(req, res, next) {
    ...
    try {
      ...
      if (user) {
        ...
        if (req.query.city) {
          query.$and.push({ city: req.query.city });
        }
        if (req.query.zip) {
          query.$and.push({ zip: req.query.zip });
        }

        if (req.query.state) {
          query.$and.push({ state: req.query.state });
        }
        ...
        if (filter) {
          ...
          filter.filter = JSON.stringify(query);
          if (req.query.rating) {
            filter.RATE = RATE
          }
          await filter.save();
          ...
          filter_.filter = JSON.stringify(query);
          filter_.RATE = RATE
          filter_.userId = req._id;
          await filter_.save();
        }
        ...
        },
        {
          "$project": {
            ...
            rating: {
              $cond: [{ $eq: [null, { $avg: "$ratings.rating" }] }, 0, { $round: [{ $avg: "$ratings.rating" }, 1] }]
            }
            // posts:1
          }
        },
        ...
        res.success({ nearby_trainer: users }, req.__(`Nearby Trainers list`))
  ...
  async Filter(req, res, next) {
    try {
      let filter = await Filter.findOne({ userId: req._id });
      if (filter) {
        return res.success({ filter }, "Successfully fetched filter!")
      } else {
        return res.warn({}, "No filter saved!")
      }
    ...
    try {
      ...
      if (user) {
        ...
        if (req.query.rating) {
          ...
          query.$and.push({ rating: { $gte: req.query.rating } });
        }
        ...
        // console.log(query);
        // console.log("------------------------------------------------------");
        let users = await User.find(query).select('name avatar');
        console.log(users);
  ...
  async createSession(req, res, next) {
    ...
    //  console.log("*************************************************************", SCHEDULEDATE, ENDDATE)
  ...
  async Book(req, res, next) {
    let { amount, tip, trainerId, sessionId, timeZone, stripeToken, cardId } = req.body;
    try {
      // console.log(req.role)
      if (req.role == 'College-Athlete' || req.role == 'Junior-Athlete') {
        let session = await TrainerSession.findOne({ _id: sessionId });
        let book_ = await Booking.findOne({ sessionId, status: true });
        // console.log("*************************************WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")
        if (book_) {
          // console.log(book_)
          // console.log(book_.athleteId, req._id)
          // console.log(book_.athleteId.equals(req._id))
          // console.log(book_.athleteId, req._id)
          if (book_.athleteId.equals(req._id)) {
            return res.warn({}, "You already booked this session>>>>>>>>>>>>>>>>>>>>>>>> !")
          } else {
            return res.warn({}, "Someone already booked this session>>>>>>>>>>>>>>>>> !")
          }
          ...
          //console.log("88888888888888888*********************************-----------------------------", mybookings)
          if (mybookings.length > 0 && mybookings[0].session) {
            ...
            if (customerData) {
              ...
              stripe.customers.createSource(
                ...
                //console.log("Ammmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm", AMOUNT)
                ...
                .then(async (charge) => {
                  ...
                  var job = new CronJob(
                    ...
                    async function () {
                      ...
                      notification.msg = `Your session about to start.`;
                      ...
                      notification_.msg = `Your session about to start.`;
                      ...
                      let msg = {
                        ...
                        "notification": {
                          ...
                          "type": "SESSION_START",
                          "body": `Your session about to start.`,
                  ...
                  var job_ = new CronJob(
                    ...
                    async function () {
                      ...
                      if (booking__ && !(booking__.start)) {
                        ...
                        notification.msg = `Your session has been cancelled.`;
                        await notification.save();
                        ...
                        notification_.msg = `Your session has been cancelled.`;
                        await notification_.save();
              ...
              stripe.customers.create({
                ...
                .then(async (customer) => {
                  ...
                  let AMOUNT = (req.body.amount) * 100;
                  //  console.log("Ammmmmmmmmmmmmmmmmmmmmmm", AMOUNT)
      ...
      let bookings = await Booking.aggregate([
        ...
        },
        {
          "$project": {
            ...
            "Athlete.hourly": "$Athlete.hourly_price",
            sessions: {
              $size: "$MyBookings"
            },
            created: "$created"
      ...
      let BookingDetails = await Booking.aggregate([
        ...
        },
        {
          "$project": {
            ...
            address: { $concat: ["$Trainer.zip", " , ", "$Trainer.city", " , ", "$Trainer.state"] },
            ADDRESS: { $concat: ["$Athlete.zip", " , ", "$Athlete.city", " , ", "$Athlete.state"] }
          }
        }
      ]);
  ...
  async CancelSession(req, res, next) {
    try {
      ...
      if (booking) {
        ...
        notification.type = 'CANCEL';
        notification.msg = `canceled your session booking.`;
        //  console.log(req.user.name)
        await notification.save();
        ...
        let msg = {
          ...
          "notification": {
            ...
            "type": "CANCEL",
            "body": `${req.user.name} cancelled your session booking.`,
          }
  ...
  async ConfirmSession(req, res, next) {
    try {
      ...
      if (booking) {
        ...
        notification.msg = `${req.user.name} confirmed your session booking.`;
        //  console.log(req.user.name)
        await notification.save();
        ...
        let msg = {
          ...
          "notification": {
            ...
            "type": "CONFIRM",
            "body": `${req.user.name} confirmed your session booking.`,
      ...
      let user = await Booking.aggregate([
        ...
        },
        {
          $project: {
            ...
            "User.avatar": "$User.avatar",
            "User.loc": "$User.loc",
            "User.address": { $concat: ["$User.zip", " , ", "$User.city", " , ", "$User.state"] },
          }
        }
      ]);
  ...
  async MakeCall(req, res, next) {
    ...
    try {
      ...
      // console.log("______________________________TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT", req.body);
      let TO = req.body.to;
      ...
      client.identity(TO);
      voiceResponse.say('WelCome WelCome ABCDEFGHIJKLMNOP');
  ...
  async AccessTokenVoice(req, res, next) {
    ...
    try {
      ...
      //   console.log(DEVICE, req.role, "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB--------------------->>>>>>>>>>>")
      let pushCredSid;
      ...
      //  console.log("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ", pushCredSid);
      ...
      //       },
      //       "push_credential_sid": pushCredSid
      //     },
      //     "identity": identity
      //   },
      //   "jti": apiSecret,
    ...
    try {
      ...
      if (user?.voip && user.role === 'Junior-Athlete') {
        ...
        note.payload = {
          ...
          note.topic = "com.WeTrain.athlete.voip";
        note.pushType = "voip";
        ...
        note.payload = {
          ...
          note.topic = "com.WeTrain.Trainer.voip";
        note.pushType = "voip";
        ...
        //  console.log("VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO VIDEO")
        ...
        //  console.log(user, deviceToken, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
        let msg = {
          "to": deviceToken,
          priority: 'high',
